...
pub async fn logout_handler(session: Session, storage: web::Data<JsonStorage>) -> impl Responder {
    // Check if this is an OAuth user
    let is_oauth = session
        .get::<String>("user_type")
    ...
    if is_oauth {
            ...
            && config.enabled
        {
            ...
            let post_logout_uri = config
                .redirect_url
                .replace("/auth_callback", "/admin/login");
